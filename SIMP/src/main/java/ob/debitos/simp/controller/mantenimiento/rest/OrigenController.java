...
@Audit(tipo = Tipo.ORIGEN, datos = Dato.ORIGEN)
@RequestMapping("/origen")
public @RestController class OrigenController
{
    ...
    {
        return origenService.buscarTodos();
    }

    @Audit(accion = Accion.Registro, comentario = Comentario.Registro)
    @PreAuthorize("hasPermission('MANT_ORIGEN', '1')")
    @PostMapping
    public ResponseEntity<?> registrarOrigen(
            @Validated({ Default.class, IRegistro.class }) @RequestBody Origen origen,
            Errors error)
    {
        if (error.hasErrors())
        {
        ...
        origenService.registrarOrigen(origen);
    ...
    @PutMapping
    public ResponseEntity<?> actualizarOrigen(
            @Validated({ Default.class, IActualizacion.class }) @RequestBody Origen origen,
            Errors error)
    {
        if (error.hasErrors())
        {
    ...
    @DeleteMapping
    public ResponseEntity<?> eliminarOrigen(
            @Validated(IActualizacion.class) @RequestBody Origen origen, Errors error)
    {
        if (error.hasErrors())
        {
            return ResponseEntity.badRequest()
        ...
        origenService.eliminarOrigen(origen);
