$(document).ready(function() {
	...
	$funcionUtil.crearSelect2($local.$tipos, "Seleccione un Tipo");
	...
	$local.tablaMantenimiento = $local.$tablaMantenimiento.DataTable({
		...
		"columns" : [ {
			...
			"data" : function(row) {
				return $funcionUtil.insertarEtiquetaSiNo(row.estrictamenteSecuencial);
			},
			"title" : "Estrictactemente Secuencial"
		}, {
			"data" : function(row) {
				return $funcionUtil.unirCodigoDescripcion(row.tipo, row.descripcionTipo);
			},
			"title" : "Tipo"
		}, {
			"data" : null,
		...
		$.ajax({
			...
			success : function(procesoAutomaticos) {
				...
				if ($local.$tablaMantenimiento.length > 0) {
					var row = $local.tablaMantenimiento.row.add(procesoAutomaticos[0]).draw();
					row.show().draw(false);
