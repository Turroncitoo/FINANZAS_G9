$(document).ready(function() {
	...
	$local.tablaMantenimiento = $local.$tablaMantenimiento.DataTable({
		"ajax": {
			"url": $variableUtil.root + "parametroSFTPProcesos?accion=buscarTodos",
			"dataSrc": ""
		},
		"language": {
	...
	$local.$modalMantenimiento.PopupWindow({
		title: "Mantenimiento de Proceso SFTP",
		autoOpen: false,
		modal: false,
		height: 280,
		width: 626,
	...
	$local.$registrarMantenimiento.on("click", function() {
		...
		$.ajax({
			type: "POST",
			url: $variableUtil.root + "parametroSFTPProcesos",
			...
			success: function(response) {
				...
				if ($local.$tablaMantenimiento.length > 0) {
					var row = $local.tablaMantenimiento.row.add(parametroSFTP).draw();
					row.show().draw(false);
	...
	$local.$tablaMantenimiento.children("tbody").on("click", ".actualizar", function() {
		...
		$local.codigoSeleccionado = parametroSFTP.codigo;
	...
	$local.$actualizarMantenimiento.on("click", function() {
		...
		var parametroSFTP = $formMantenimiento.serializeJSON();
		parametroSFTP.codigo = $local.codigoSeleccionado;
		$.ajax({
			type: "PUT",
			url: $variableUtil.root + "parametroSFTPProcesos",
			data: JSON.stringify(parametroSFTP),
	...
	$local.$tablaMantenimiento.children("tbody").on("click", ".eliminar", function() {
		...
		$.confirm({
			...
			title: "Aviso",
			content: "\u00BFDesea eliminar el proceso <b>'" + parametroSFTP.codigo + " - " + parametroSFTP.descripcion + "'</b>?",
			buttons: {
				Aceptar: {
					action: function() {
								...
								$.ajax({
									...
									switch (xhr.status) {
										case 400:
											...
											break;
										case 409:
											var mensaje = $funcionUtil.obtenerMensajeError(" El proceso <b>" + parametroSFTP.codigo + " - " + parametroSFTP.descripcion + "</b>", xhr.responseJSON, $variableUtil.accionEliminado);
