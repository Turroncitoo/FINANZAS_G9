$(document).ready(function() {
	...
	$local.tablaResultados = $local.$tablaResultados.DataTable({
		...
		"columns": [
			...
			{
				"data": function(row) {
					return $funcionUtil.unirCodigoDescripcion(row.idEstadoTarjeta, row.descripcionEstadoTarjeta)
				},
	...
	$local.$btnBuscar.on('click', function() {
		...
		$.ajax({
			type: "GET",
			url: $variableUtil.root + "reporte/estadoTarjeta/resumenMensual?accion=buscar",
	...
	$local.$btnExportar.on("click", function() {
		...
		window.location.href = $variableUtil.root + "reporte/estadoTarjeta/resumenMensual?accion=exportar&" + paramCriterioBusqueda;
	});
